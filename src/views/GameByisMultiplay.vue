...
const getGameByMultiplay = async (mul) => {
    console.log(mul)
    if(mul == 'true'){
        const qry = query(gamesRef, where("isMultiplayer", "==", true))
        await getQuery(qry)
    }else if (mul == 'false') {
        const qry = query(gamesRef, where("isMultiplayer", "==", false))
        await getQuery(qry)
    }
...
onMounted(() => {
    getGameByMultiplay(route.params.id)
})
</script>
            ...
            <RouterLink :to="{ name: 'gamedetail', params: { id: game.title } }">
                ...
                <div class="flex space-x-2 justify-center mt-1">
                    <div v-for="gamegenre in game.genre" :key="gamegenre" class="bg-red-200">
                        <RouterLink :to="{ name: 'gamesbyelse', params: { id: gamegenre } }">{{ gamegenre }}</RouterLink>
                    </div>
